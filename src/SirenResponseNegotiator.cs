...
public class SirenResponseNegotiator : IResponseNegotiator
{
    ...
    public async Task Handle<T>(HttpRequest req, HttpResponse res, T model, CancellationToken cancellationToken)
    {
        var responseGenerators =
            req.HttpContext.RequestServices.GetServices(typeof(ISirenResponseGenerator)) as
                IEnumerable<ISirenResponseGenerator>;
        ...
        var body = JsonSerializer.Serialize(response, _serializerOptions);

        res.ContentType = "application/vnd.siren+json";
        await res.WriteAsync(body, cancellationToken: cancellationToken);
    }
